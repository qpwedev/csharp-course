...
namespace Model
{
	...
	public class ShoppingCart {
		...
		public decimal GetTotal(ModelStore store){
			decimal total = 0;
		...
		public ShoppingCartItem GetItem(int bookId) => this.Items.Find(item => item.BookId == bookId);

		public bool RemoveBook(int bookId){
			ShoppingCartItem item = this.GetItem(bookId);
			if (item != null){
				item.Count--;
				if (item.Count <= 0){
		...
		public void AddBook(int bookId){
			...
			else this.Items.Add(new ShoppingCartItem { BookId = bookId, Count = 1 });
		}
	}
}
